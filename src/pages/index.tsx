...
export function DashboardPage() {
  ...
  const cards = [
    ['Active Clients', metrics.activeClients, 'from-emerald-500 to-emerald-700', Users],
    ['Total Active Loans', metrics.activeClients, 'from-blue-500 to-blue-700', TrendingUp],
    ['Total Money Lent', formatUGX(metrics.totalLent), 'from-violet-500 to-violet-700', TrendingUp],
    ['Outstanding Balance', formatUGX(metrics.outstanding), 'from-amber-500 to-orange-700', AlertCircle],
    ['Money Collected', formatUGX(metrics.collected), 'from-green-500 to-emerald-700', CheckCircle2],
  ];
  return <div className="space-y-4"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">{cards.map(([label, value, gradient, Icon]) => <Card key={String(label)} className={`fade-in border-2 text-white bg-gradient-to-r ${gradient}`}><div className="flex items-center justify-between"><p className="text-sm">{label}</p><Icon /></div><p className="mt-4 text-2xl font-bold">{value as string}</p></Card>)}</div><Card><h3 className="mb-3 font-semibold">Recent Payments</h3><table className="w-full text-sm"><thead><tr className="text-left"><th>Date</th><th>Client</th><th>Amount</th><th>Status</th></tr></thead><tbody>{transactions.slice(0, 6).map((t) => <tr key={t.id}><td>{t.date}</td><td>{t.clientName}</td><td>{formatUGX(t.amount)}</td><td><Badge tone="success">Paid</Badge></td></tr>)}</tbody></table></Card></div>;
}
...
export function AddClientPage() {
  ...
  const submit = (e: FormEvent) => { e.preventDefault(); if (!user) return; addClient({ fullName, phoneNumber, nationalId, location, guarantorName, guarantorId, guarantorPhone, guarantorLocation, loanAmount, addedBy: user.email, startDate: '', endDate: '' }); };
